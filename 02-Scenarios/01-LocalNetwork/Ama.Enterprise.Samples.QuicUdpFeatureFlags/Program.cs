...
internal sealed class Program
{
    public static async Task Main(string[] args)
    {
        ...
        {
            Console.WriteLine("WARNING: QUIC is not natively supported on this platform or missing prerequisites (e.g., libmsquic).");
            Console.WriteLine("The P2P network will fail to bind. Press any key to exit...");
            Console.ReadKey();
            return;
        }
        ...
        services.AddDistributedCrdtP2p("feature-flags-quic-mesh", replicaId);

        // 4. Add the Feature Flags services.
        services.AddFeatureFlags();

        // Load Certificates and Wire Encryption Key from disk
        var certPath = "cluster.pfx";
        var certPassword = "local-test-password";
        
        var authCertPath = "auth.pfx";
        var authCertPassword = "auth-test-password";
        
        var encryptionKeyPath = "encryption.key";

        if (!File.Exists(certPath) || !File.Exists(authCertPath) || !File.Exists(encryptionKeyPath))
        {
            Console.WriteLine("Error: Required certificate or encryption key files are missing.");
            Console.WriteLine($"Please ensure '{certPath}', '{authCertPath}', and '{encryptionKeyPath}' exist in the application directory.");
            Console.WriteLine("Press any key to exit...");
            Console.ReadKey();
            return;
        }

        using var clusterCert = X509CertificateLoader.LoadPkcs12FromFile(certPath, certPassword);
        using var authCert = X509CertificateLoader.LoadPkcs12FromFile(authCertPath, authCertPassword);
        var authCertBytes = authCert.Export(X509ContentType.Cert);
        var encryptionKey = File.ReadAllText(encryptionKeyPath).Trim();

        // 5. Configure the Mesh Network to use QUIC transport and UDP discovery.
        services.AddP2pMesh("feature-flags-quic-mesh")
                ...
                .AddQuicTransport(options =>
                {
                    ...
                    options.RemoteCertificateValidationCallback = (sender, cert, chain, sslPolicyErrors) =>
                    {
                        ...
                        // Validate that the remote peer's transport certificate matches our cluster's expected thumbprint
                        var remoteThumbprint = cert.GetCertHashString();
                        return string.Equals(remoteThumbprint, clusterCert.Thumbprint, StringComparison.OrdinalIgnoreCase);
                    };
                })
        ...
        var logger = provider.GetRequiredService<ILoggerFactory>().CreateLogger("QuicUdpSample");
        ...
        var app = new InteractiveConsoleApp(logger, "QUIC/UDP Feature Flags Showcase", cts);
        app.SetCurrentPort(currentPort);
        app.SetIntroText("This application demonstrates multiplexed QUIC transport and UDP discovery to sync feature flags across clustered nodes natively.");
        ...
        logger.LogInformation("Starting QUIC/UDP Node on port {Port}...", currentPort);
